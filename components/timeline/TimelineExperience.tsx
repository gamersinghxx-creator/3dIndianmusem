...
export default function TimelineExperience({ catalog }: { catalog: CatalogProp }) {
  ...
  return (
    <main className="relative flex h-screen w-screen flex-col overflow-hidden">
      {/* Header */}
      <header className="z-20 flex flex-wrap items-center gap-x-6 gap-y-3 border-b border-white/10 bg-ink/80 px-6 py-3 backdrop-blur">
        <div className="mr-auto">
          ...
          >
            Antarang
            <span className="ml-2 text-xs font-sans uppercase tracking-[0.3em] text-gold/70">
        ...
        <input
          ...
          className="w-56 rounded-full border border-white/15 bg-transparent px-4 py-1.5 text-xs text-ivory placeholder:text-white/30 outline-none focus:border-gold"
        />
      </header>
      ...
      {/* data provenance */}
      <div className="pointer-events-none absolute right-4 top-16 z-10 text-[10px] text-white/25">
